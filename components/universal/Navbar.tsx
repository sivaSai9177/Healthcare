        ...
        style={[
          ({ pressed }) => ({
            ...
            backgroundColor: pressed 
              ...
              ? theme.accent 
              : 'transparent',
            opacity: pressed && !animated ? 0.8 : 1,
          }),
  ...
  return (
    <AnimatedView
      entering={Platform.OS !== 'web' && animated && isAnimated && shouldAnimate() && animationType === 'slide' 
        ? SlideInLeft.duration(config.duration.normal) 
        : undefined}
      style={{
        height: 64,
...
const SidebarVariant: React.FC<NavbarProps & { isOpen?: boolean; onToggle?: () => void }> = ({ 
  ...
  useEffect(() => {
    if (animated && isAnimated && shouldAnimate()) {
      sidebarWidth.value = withSpring(isOpen ? 240 : 60, config.spring);
      contentOpacity.value = withTiming(isOpen ? 1 : 0, { duration: config.duration.normal });
    }
  }, [isOpen, animated, isAnimated, shouldAnimate, config]);
          ...
          <HStack alignItems="center" justifyContent="space-between">
            ...
            {hasChildren && isOpen && (
              <Symbol
                ...
                style={[
                  Platform.OS !== 'web' && animated && isAnimated && shouldAnimate() ? chevronStyle : {},
                  Platform.OS === 'web' && animated && isAnimated && shouldAnimate() 
        ...
        {hasChildren && isExpanded && isOpen && (
          <AnimatedView
            ...
            exiting={Platform.OS !== 'web' && animated && isAnimated && shouldAnimate() 
              ? FadeOut.duration(config.duration.fast) 
              : undefined}
          >
            <VStack>
  ...
  const animatedSidebarStyle = useAnimatedStyle(() => ({
    width: animated && isAnimated && shouldAnimate() ? sidebarWidth.value : (isOpen ? 240 : 60),
  }));
  ...
  return (
    <AnimatedBox
      ...
      style={[
        ...
        Platform.OS !== 'web' && animated && isAnimated && shouldAnimate() ? animatedSidebarStyle : {},
        Platform.OS === 'web' && animated && isAnimated && shouldAnimate() && {
      ...
      <Box p={3 as SpacingScale} borderBottomWidth={1} borderTheme="border">
        <HStack justifyContent="space-between" alignItems="center">
          {isOpen ? (
            ...
            <Symbol name="apps" size={24} color={theme.primary} />
          )}
          {onToggle && (
            <Button
              variant="ghost"
