...
import { ensureOrganizationHospitals } from './ensure-organization-hospitals';
...
async function setupHealthcareComplete() {
  ...
  try {
    // Create hospitals table with organization relationship
    ...
    log.info('Inserting default organization...', 'SETUP');
    await db.execute(sql`
    ...
    // Insert default hospital with organization_id
    log.info('Inserting default hospital...', 'SETUP');
    await db.execute(sql`
      INSERT INTO hospitals (id, organization_id, name, code, address, contact_info, is_default)
      VALUES (
        'f155b026-01bd-4212-94f3-e7aedef2801d',
        'f155b026-01bd-4212-94f3-e7aedef2801d', -- Using same ID for org and hospital for simplicity
    ...
    log.info('Adding default_hospital_id column to user table...', 'SETUP');
    await db.execute(sql`
      ALTER TABLE "user" 
      ...
      try {
        await db.update(user)
          .set({ 
            role: update.role,
            name: update.name,
            organizationId: 'f155b026-01bd-4212-94f3-e7aedef2801d' // Assign to default hospital
          })
          .where(eq(user.email, update.email));
        
        log.info(`Updated ${update.email} to role: ${update.role} with organization`, 'SETUP');
      } catch (error) {
    ...
    log.info('Adding users to organization...', 'SETUP');
    await db.execute(sql`
      ...
      SELECT 
        ...
        CASE 
          ...
          WHEN role = 'head_doctor' THEN 'manager'
          ELSE 'member'
        END
      FROM "user" 
      WHERE email IN ('johncena@gmail.com', 'doremon@gmail.com', 'johndoe@gmail.com', 'saipramod273@gmail.com')
    ...
    // Ensure all organizations have hospitals
    log.info('Ensuring all organizations have hospitals...', 'SETUP');
    await ensureOrganizationHospitals();
    ...
    log.info('✅ All organizations have default hospitals', 'SETUP');
    log.info('', 'SETUP');
