...
export function useAlertSubscription({
  ...
  useEffect(() => {
    ...
    // Try to use WebSocket subscription first
    try {
      // Import tRPC API dynamically to avoid circular dependencies
      import('@/lib/api/trpc').then(({ api }) => {
        const subscription = api.healthcare.subscribeToAlerts.useSubscription(
          { hospitalId },
          {
            ...
            onError: (err) => {
              log.error('WebSocket subscription error, falling back to polling', 'ALERT_SUBSCRIPTION', err);
        ...
        if (subscription) {
          ...
          log.info('Using WebSocket for alert updates', 'ALERT_SUBSCRIPTION');
        } else {
          // Fall back to polling if subscription fails
          startPolling();
        }
      }).catch((error) => {
        log.error('Failed to import tRPC API, falling back to polling', 'ALERT_SUBSCRIPTION', error);
        startPolling();
      });
    } catch (error) {
      log.error('Failed to setup WebSocket subscription, falling back to polling', 'ALERT_SUBSCRIPTION', error);
      startPolling();
    }
    ...
    return () => {
      ...
      } else if (subscriptionRef.current?.type === 'websocket') {
        // WebSocket cleanup handled by tRPC
      }
