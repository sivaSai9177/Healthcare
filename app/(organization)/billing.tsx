...
import { BillingBlock } from '@/components/blocks/organization';
...
export default function OrganizationBilling() {
  ...
  if (!user || (user.role !== 'admin' && user.role !== 'manager')) {
    log.warn('Non-admin/manager tried to access organization billing', 'ORG_BILLING', { 
      userId: user?.id, 
      role: user?.role 
    });
  ...
  const handleUpgrade = (plan: string) => {
    log.info('Upgrading plan', 'ORG_BILLING', { organizationId, plan });
    // Handle plan upgrade
  };
  
  const handleCancel = () => {
    log.info('Cancelling plan', 'ORG_BILLING', { organizationId });
    // Handle plan cancellation
  };
  
  const handleUpdatePayment = () => {
    log.info('Updating payment method', 'ORG_BILLING', { organizationId });
