...
export default function AlertsScreen() {
  ...
  const renderAlert = (alert: Alert) => (
    <Card
      ...
      onPress={() => router.push(`/(zmodals)/patient-details?alertId=${alert.id}`)}
    >
      <VStack space={spacing.sm}>
        <HStack space={spacing.md} style={{ justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          {alert.status === 'acknowledged' ? (
            ...
            <TrendingUp size={20} color={theme.colors.destructive} />
          ) : (
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      {user?.role === 'operator' && (
        <View style={{
          ...
          <Button
            size="lg"
            onPress={() => router.push('/(zmodals)/create-alert')}
            style={{
              ...(Platform.OS === 'web' && {
