...
import SignupScreen from '@/app/(public)/auth/register';

// Mock dependencies
jest.mock('@/hooks/useAuth', () => ({
  useAuth: () => ({
    updateAuth: jest.fn<any>(),
    setLoading: jest.fn<any>(),
    setError: jest.fn<any>(),
  }),
}));

jest.mock('@/lib/core/alert', () => ({
  showErrorAlert: jest.fn<any>(),
  showSuccessAlert: jest.fn<any>(),
}));
...
jest.mock('@/lib/trpc', () => ({
  api: {
    auth: {
      signUp: {
        useMutation: () => ({
          mutateAsync: jest.fn<any>().mockResolvedValue({
            success: true,
